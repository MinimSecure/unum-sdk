...
$(dirname "$BASH_SOURCE")/docker_network.sh "$container_name" "$ifname_wan" "$wlan_phyname" || \
    echo "---> failed to bring up docker networks, continuing anyway"

# Replace the current process with another bash session to explore the container.
echo "---> starting bash session to take a look around"
...
echo
echo "  2. Set the container MAC address to the MAC address generated by the"
echo "     Minim application:"
echo "         ifconfig eth2 hw ether <generated MAC>"
echo
echo "  3. Configure and start all services, including Unum:"
echo "         startup.sh"
echo
