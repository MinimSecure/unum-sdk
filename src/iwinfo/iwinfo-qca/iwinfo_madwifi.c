		...
		if( get80211priv(ifname, IEEE80211_IOCTL_GETCHANINFO, &chans, sizeof(chans)) >= 0 )
		{
			*buf = 0;
			...
			for( i = 0; i < chans.ic_nchans; i++ )
			{
				...
				{
					*buf = ieee80211_mhz2ieee(chans.ic_chans[i].ic_freq);
					break;
				}
			}
...
iswmeoui(const u_int8_t *frm)
{
	...
	static int
is_wme(const u_int8_t *vp, int ielen)
{
	while (ielen > 0) {
		if (vp[0] == IEEE80211_ELEMID_VENDOR) {
			if (iswmeoui(vp)) {
				return 1;
			}
		}
...
static int get_stats(const char *dev,
		struct ieee80211req_sta_info *si,
		struct iwinfo_assoclist_entry *entry)
{
	...
	if( madwifi_wrq(&iwr, dev, IEEE80211_IOCTL_STA_STATS, NULL, 0) >= 0 ) {
		entry->rx_bytes = ns->ns_rx_bytes;
		entry->tx_bytes = ns->ns_tx_bytes;
		entry->rx_packets = ns->ns_rx_data;
		entry->tx_packets = ns->ns_tx_data;
		entry->rx_drop_misc = ns->ns_rx_decap +
			ns->ns_rx_decryptcrc +
			ns->ns_rx_tkipicv +
			ns->ns_rx_wepfail +
			ns->ns_rx_tkipmic +
			ns->ns_rx_ccmpmic +
			ns->ns_rx_wpimic;
		entry->tx_failed = ns->ns_tx_discard;
		entry->tx_retries = 0;
		for (ii = 0; ii < WME_NUM_AC; ii++) {
			entry->tx_retries += ns->ns_excretries[ii];
		}
		return 0;
	} else {
		return -1;
...
static int madwifi_get_assoclist(const char *ifname, char *buf, int *len)
{
	...
	// For no results, a value of 12 is being returned. So compare against 12
	if( (tl = get80211priv(ifname, IEEE80211_IOCTL_STA_INFO, tmp, 24*1024)) > 12 )
	{
		cp = tmp;
		bl = 0;
		...
		do {
			...
			// Vendor IEs start at offset 230
			vp = (u_int8_t *)(si) + 230;
			...
			entry.rx_rate.mcs = maxrate;
			entry.tx_rate.mcs = maxrate;
			if (si->isi_stamode >= 15) {
				entry.rx_rate.is_vht = 1;
				entry.tx_rate.is_vht = 1;
				if (si->isi_stamode == 15) {
					entry.tx_rate.mhz = 20;
					entry.rx_rate.mhz = 20;
				} else if (si->isi_stamode <= 18) {
					entry.tx_rate.mhz = 40;
					entry.rx_rate.mhz = 40;
				} if (si->isi_stamode == 19) {
					entry.tx_rate.mhz = 80;
					entry.rx_rate.mhz = 80;
				} else {
					entry.tx_rate.mhz = 160;
					entry.rx_rate.mhz = 160;
				}
			} else if (si->isi_stamode >= 7) {
			...
			entry.tx_rate.nss = (si->isi_nss & 0x0f);
			entry.rx_rate.nss = ((si->isi_nss >> 4) & 0x0f);

			entry.rx_rate.is_short_gi = (si->isi_htcap & (IEEE80211_HTCAP_C_SHORTGI40 | IEEE80211_HTCAP_C_SHORTGI20));
			entry.tx_rate.is_short_gi = (si->isi_htcap & (IEEE80211_HTCAP_C_SHORTGI40 | IEEE80211_HTCAP_C_SHORTGI20));

			entry.tx_rate.is_40mhz = (si->isi_htcap & (IEEE80211_HTCAP_C_CHWIDTH40));

			// An entry in this list means authorized
			entry.is_authorized = 1;
			entry.is_authenticated = 1;

			entry.is_wme = is_wme(vp, si->isi_ie_len);
			get_stats(ifname, si, &entry);
...
int madwifi_get_hwmodelist (const char *ifname, int *buf)
{
	...
	if( !(wifidev = madwifi_isvap(ifname, NULL)) )
	{
		wifidev = (char *)ifname;
	}
