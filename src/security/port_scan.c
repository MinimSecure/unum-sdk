...
#define POST_SCAN_RSP_URL "%s/v2/lans/%s/devices/%s/port_info"
...
// through the 'ipv4' pointer. The util_platform_enum_ifs() then returns
...
static int scan_device(PORT_SCAN_DEVICE_t *port_scan_device)
{
    ...
    for(;;) {
        ...
        if(util_platform_enum_ifs(UTIL_IF_ENUM_RTR_LAN,
